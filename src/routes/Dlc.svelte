...
<script>
  ...
  async function onSubmit(e) {
    ...
    if(result !== null && result.status !== 'error'){
      ...
      visible = true
      saveOpenPosition(result.id, result.hashX, 0);
    }
  ...
  async function getPosition(payment_hash) {
    ...
    const res = await fetch(BASE_URL_DLC + `/dlc?` + new URLSearchParams({
      hashX: payment_hash,
    }))
  ...
  async function getSavedPositions() {
    for (const key in localStorage) {
      if (localStorage.hasOwnProperty(key)) {
        let item = JSON.parse(JSON.parse(JSON.stringify(localStorage.getItem(key))));
        if(item.closed == 0){
          let res = await getPosition(item.hashX);
          listOpenPosition.push(res);
          if(res.status === 'SETTLED' || res.status === 'CANCELED'){
            saveOpenPosition(key, item.hashX, 1)
          }
        }else if(item.closed == 1){
          let res = await getPosition(item.hashX);
          listClosedPosition.push(res);
        }
      }
    }
    position2 = true;
  }
  function saveOpenPosition(id, hashX, closed){
    let obj = {
      id : id,
      hashX : hashX,
      closed : closed
  ...
  // This is called from update button
  /*
  async function updateSavedPositions(){
    for (const key in localStorage) {
      if (localStorage.hasOwnProperty(key)) {
...
<p><a href="javascript:void(0);" on:click={() => (open = true)}>Check the status</a></p>
</main>
